...
window.formatMoney = function (money) {
  if (money === 0) {
    return '0 元'
  } else {
    if (money && money !== '') {
      let temp = Number(money)
      if (_.isNumber(temp)) {
        return (temp / 100) + ' 元'
      } else {
        return ''
...
window.formatSpeDate = function (str) {
  if (str) {
    let tempStr = str.replace('T', ' ')
    return formatDate(new Date(tempStr).getTime(), 'YYYY-MM-DD H:i:s')
  } else {
    return ''
  }
}
