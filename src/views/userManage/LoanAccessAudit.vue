        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <el-button type='success'
                       ...
                       v-permission='["platform:member:enteredAudit:audit"]'
                       @click='openAuditModal(scope.$index, scope.row)'
                       size="mini">
              准入审核
            </el-button>

            <span v-permission='["platform:member:enteredAudit:list"]'>
            <el-popover
              placement="left"
              width="500"
              v-if='scope.row.enterStatus!="processing"'
...
<script>
  export default {
    ...
    methods: {
      formatDate,
      getLimit() {
        AXIOS.post('/auth/platformMember/memberEnteredAuditInit').then(res => {
          this.min = Number(res.enterBorrowLimitMin) || 500
          this.max = Number(res.enterBorrowLimitMax) || 1000
        })
      },
      goAuthInfo(id) {
