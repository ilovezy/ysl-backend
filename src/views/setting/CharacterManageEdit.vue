  ...
  export default {
    ...
    methods: {
      ...
      loadTreeData(isEdit, target) {
        ...
        AXIOS.post('/auth/menu/viewAllMenuTree', param).then(res => {
          this.treeData = isLongArr(res) ? res : []
          this.loading = false
          if (isEdit) {
            this.getUserInfo()
          }
      ...
      submitForm: _.debounce(function (formName) {
        if (this.validForm()) {
          let apiUrl = '/auth/role/createRole'
          let successTip = '角色添加成功'
          if (this.id && this.id != 'add') {
            apiUrl = '/auth/role/editRole'
            successTip = '角色修改成功'
          }
