<template>
  <div class="app-container">
    ...
    <div class='main-table-wrap'>
      ...
      <el-row :gutter='50'>
        <el-col :span='12'
                v-permission='["platform:authority:menu:view"]'
  ...
  export default {
    ...
    methods: {
      ...
      addMenu() {
        ...
        if (this.validForm()) {
          AXIOS.post('/auth/menu/createMenu', this.formData).then(res => {
            this.loadData()
      ...
      deleteMenu() {
        ...
        if (id) {
          this.$confirm('确定要删除该菜单？', {
            ...
            AXIOS.post('/auth/menu/deleteMenu', {
              id
            }).then(res => {
      ...
      editMenu() {
        AXIOS.post('/auth/menu/editMenu', this.formData).then(res => {
      ...
      loadData() {
        this.loading = true
        AXIOS.post('/auth/menu/viewAllMenuTree', {
          target: this.currentTarget
        }).then(res => {
          this.data = isLongArr(res) ? res : []
          this.resetNewMenu()
          this.loading = false
        }).catch()
      }
